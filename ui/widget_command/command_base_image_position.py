...
def normalize_image_path(abs_path: str) -> str:
    """将图片路径归一化为相对于项目根的路径
    若图片在 configs 目录外，则复制到当前配置目录
    返回相对路径（如 configs/默认/xxx.png）"""
    if not abs_path:
        return ''
    abs_path = os.path.normpath(os.path.abspath(abs_path))
    config_dir = os.path.normpath(os.path.abspath(get_current_config_image_dir()))
...
class HBoxLayoutGroup1(QHBoxLayout):
    ...
    def __init__(self):
        ...
        self.label_show_pic.setScaledContents(True)  # Qt 自动缩放，避免 0×0 问题
        self.label_show_pic.setMinimumSize(40, 30)
        ...
        self.label_conf = QLabel('匹配置信度')
        self.addWidget(self.label_conf)

        self.doubleSpinBox_confidence = QDoubleSpinBox()
        self.doubleSpinBox_confidence.setButtonSymbols(QAbstractSpinBox.NoButtons)
        self.doubleSpinBox_confidence.setRange(0.1, 1.0)
        self.doubleSpinBox_confidence.setSingleStep(0.05)
        self.doubleSpinBox_confidence.setValue(default_confidence)
        self.doubleSpinBox_confidence.setToolTip('图像匹配精确度，越低越宽松，0.7-0.9 通常可用')
        self.addWidget(self.doubleSpinBox_confidence)
        ...
        self.doubleSpinBox_confidence.valueChanged.connect(self.args_changed)
    ...
    def choose_pic(self, image_path=None):
        ...
        if not image_path:
            default_dir = get_current_config_image_dir()
            image_path, _ = QFileDialog.getOpenFileName(self.parentWidget(), "选择图片", default_dir,
                                                        "图片文件(*.png *.jpg *.bmp)")

        if image_path:
            stored_path = normalize_image_path(image_path)
            self.label_show_pic.setProperty('image_path', stored_path)
            # Qt setScaledContents(True) 自动按比例缩放，无需手动计算
            display_path = resolve_image_path(stored_path)
    ...
    def get_screenshot_area(self, screenshot_area: list):
        ...
        save_dir = get_current_config_image_dir()
        save_pic_file = os.path.normpath(os.path.join(save_dir, image_name))
