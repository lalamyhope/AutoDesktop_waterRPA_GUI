...
class CommandClickImagePosition(QWidget):
    ...
    def load_args(self, args_dict):
        ...
        # 使用确定性路径解析（基于项目根，不依赖 cwd）
        if screenshot_image_path:
            screenshot_image_path = resolve_image_path(screenshot_image_path)
        ...
        self.ui_move.doubleSpinBox_confidence.setValue(args_dict.get('confidence', default_confidence))
    ...
    def check_args(self):
        ...
        if pic_file:
            pic_file = resolve_image_path(pic_file)
    ...
    def send_args(self):
        ...
        self.args_dict['confidence'] = self.ui_move.doubleSpinBox_confidence.value()
