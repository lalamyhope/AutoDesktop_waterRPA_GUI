...
图标
"""
# 仅保留程序图标（打包用），其余按钮/状态已改用 UTF-8 字符
icon_main = r'icon/main.ico'
...
command_config = 'widget_command.json'  # 配置文件名
screenshot_folder = 'screenshot_temp'  # 临时截图文件存放文件夹
global_config = 'global.json'  # 全局设置文件名
...
default_use_clipboard = False  # 默认使用剪贴板现有内容（False=使用输入文本）
default_hotkey = ''  # 默认热键
default_keys = ''  # 默认键
default_key = ''  # 默认键
default_x = 1  # 默认x轴坐标值
default_y = 1  # 默认y轴坐标值
default_step_x = 0  # 默认X轴每轮步进值
default_step_y = 0  # 默认Y轴每轮步进值
...
command_chs_to_en_dict = {'': '',
                          ...
                          '键盘操作-快捷键': 'command_key_in_hotkey',
...
command_en_to_chs_dict = {'': '',
                          ...
                          'command_key_in_hotkey': '键盘操作-快捷键',
